...
from basicas import preprocesa, insercion
...
def predict():
    ...
    probability = np.max(model.predict_proba(final_features) )* 100 # Calcula las probabilidades
    output = round(prediction[0], 2)
    if (output == 1):
        out = "Yes"
    else:
        out = "No"
    return render_template('formulario.html',prediction_text='Chance de tomar la campaña de temporada: {} con un % de {}'.format(out,probability))
#
